...
class SellerLedger_Transaction_Queries {

	public static function active_ids( $limit = 0 ) {
		global $wpdb;

		$table = SellerLedger_Transaction::table_name();
	...
	public static function reify( $results ) {
		...
		foreach ( (array) $results as $result ) {
			...
			$records[] = SellerLedger_Transaction::populate( new $type(), $result );
		}
	...
	public static function klass_from_type( $type ) {
		return ( 'order' === $type ? 'SellerLedger_Transaction_Order' : 'SellerLedger_Transaction_Refund' );
	}
}
